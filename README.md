...
**Future Iteration Path:** To mitigate these compute limitations, future deployments should reconfigure the Gunicorn WSGI server to utilize asynchronous worker classes (such as gevent) to handle concurrent network I/O more efficiently. However, the most desirable architectural evolution involves a complete decoupling of the infrastructure. This includes migrating the compute tier to Azure Kubernetes Service (AKS) paired with an Azure Load Balancer to create a true distributed cluster, and offloading the data tier to a managed Azure Database for MySQL instance. This evolution would allow the backend web containers to scale horizontally across multiple independent Virtual Machines while ensuring enterprise-grade high availability for the database, entirely bypassing the current single-machine bottlenecks.
```
