# GSU\_GRADUATE\_DEGREE\_PLANNER
...
cd gsu\_graduate\_degree\_planner
...
\* `gsu\_graduate\_degree\_planner-load\_balancer-1` (Mapped to Port 80)

\* `gsu\_graduate\_degree\_planner-web1-1` (Port 5000)

\* `gsu\_graduate\_degree\_planner-web2-1` (Port 5000)

\* `gsu\_graduate\_degree\_planner-db-1` (Port 3306)
...
4\. \*\*Access:\*\* The application will be live at `http://<YOUR\_AZURE\_PUBLIC\_IP>`.
...
docker logs -f gsu\_graduate\_degree\_planner-web1-1
...
docker logs -f gsu\_graduate\_degree\_planner-load\_balancer-1
...
locust -f locustfile\_baseline.py
...
&#x20;  \* For production testing: `http://<YOUR\_AZURE\_PUBLIC\_IP>` (Do not include a trailing slash or port number).
